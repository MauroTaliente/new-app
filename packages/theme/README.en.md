...
React theme **runtime**: in-memory state (context) + **DOM sync** (`body` / `documentElement` / `data-theme`), with **`onThemeChange`** for persistence (`@lib/persistence`, cookies, Server Actions, etc.).

Theme **names** are defined in **CSS → `lib-styles-generate` → `styles.generated.ts`** (single source of truth).
...
- `@lib/context` — theme state
- `@lib/styles` — `ExtractThemeName` / `ExtractDefaultTheme` aligned with generated `styles`

Typical install in an app that already uses `@lib/styles` and `@lib/persistence`:
...
`@lib/theme` does **not** import `next/headers`. Patterns:

### A — Cookie readable on the client (like the Vite demo)

Client entry: `getInitialTheme(defaultTheme)` + `onThemeChange` writing cookie / `localStorage` via `@lib/persistence`.
...
| `ExtractThemeName`, `ExtractDefaultTheme` | Re-exported from `@lib/styles` |
